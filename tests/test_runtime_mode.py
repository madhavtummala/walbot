...
from src.core.bot_runtime import _deployment_enabled
...
def test_there_is_no_process_wide_runtime_mode() -> None:
    ...
    A --mcp process mode could only contradict the deployment's own schedule, and it did: the
...
def test_an_agent_driven_deployment_is_never_scheduled() -> None:
    """It is switched on, but it waits for an external request rather than a clock."""
    controls = {"deployments": [{"algorithm": "dca", "account_id": "paper",
                                 "enabled": True, "cron": ""}]}

    assert _deployment_enabled("dca")(controls) is False


def test_a_deployment_with_a_cron_is_scheduled() -> None:
    controls = {"deployments": [{"algorithm": "dca", "account_id": "paper",
                                 "enabled": True, "cron": "*/15 9-15 * * 1-5"}]}

    assert _deployment_enabled("dca")(controls) is True


def test_the_mcp_server_is_always_started() -> None:
    """No switch to turn it off: an agent-driven deployment needs something to call into.

    Whether an algorithm is driven by an agent is whether its deployment has a ``cron``, so a
    process-wide off switch could only contradict it -- and an agent-driven deployment with
