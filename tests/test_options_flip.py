...
class TestMacroTrend:
    def test_bull_when_above_ma_and_positive_return(self) -> None:
        ...
        cfg = OptionsFlipConfig(trend_ma_period=50, trend_lookback_days=5, trend_min_return=0.0)
        assert detect_macro_trend(bars, cfg)[0] == "bull"

    def test_bear_when_below_ma_and_negative_return(self) -> None:
        closes = [150.0 - i * 0.5 for i in range(60)]
        bars = _make_bars(closes)
        cfg = OptionsFlipConfig(trend_ma_period=50, trend_lookback_days=5, trend_min_return=0.0)
        assert detect_macro_trend(bars, cfg)[0] == "bear"

    def test_flat_when_mixed_signals(self) -> None:
        closes = [100.0] * 60
        bars = _make_bars(closes)
        cfg = OptionsFlipConfig(trend_ma_period=50, trend_lookback_days=5, trend_min_return=0.0)
        assert detect_macro_trend(bars, cfg)[0] == "flat"

    def test_flat_with_insufficient_data(self) -> None:
        bars = _make_bars([100.0, 101.0, 102.0])
        cfg = OptionsFlipConfig(trend_ma_period=50)
        assert detect_macro_trend(bars, cfg)[0] == "flat"

    def test_min_return_threshold_filters_noisy_markets(self) -> None:
        closes = [100.0 + i * 0.01 for i in range(60)]
        bars = _make_bars(closes)
        cfg = OptionsFlipConfig(trend_ma_period=50, trend_lookback_days=5, trend_min_return=0.02)
        assert detect_macro_trend(bars, cfg)[0] == "flat"
...
class TestCandidateScoring:
    ...
    def test_price_outside_range(self) -> None:
        cfg = OptionsFlipConfig(min_price=50, max_price=200)
...
class TestOptionPricing:
    def test_basic_pricing(self) -> None:
        cfg = OptionsFlipConfig(entry_discount_pct=0.05, exit_target_pct=0.30)
...
class TestStrikeSelection:
    def test_call_strike_near_atm(self) -> None:
        cfg = OptionsFlipConfig(call_delta_min=0.35, call_delta_max=0.55)
        strike = _select_strike(100.0, "call", cfg)
        assert 40.0 < strike < 70.0

    def test_put_strike_near_atm(self) -> None:
        cfg = OptionsFlipConfig(put_delta_min=-0.55, put_delta_max=-0.35)
...
class TestOptionsFlipAlgorithm:
    def test_algorithm_class(self) -> None:
        algo = OptionsFlipAlgorithm({})
        assert algo.algorithm_id == "options_flip"

    def test_flat_macro_no_intents(self) -> None:
        algo = OptionsFlipAlgorithm({})
        benchmark = _make_bars([100] * 60)
        context = AlgorithmContext(
            config=OptionsFlipConfig(benchmark_symbol="SPY", trend_ma_period=50),
            daily_bars_by_symbol={"SPY": benchmark},
        )
        plan = algo.plan(context)
        assert plan.intents == []
        assert plan.metadata.get("macro") == "flat"
    ...
    def test_bull_macro_picks_candidate(self) -> None:
        ...
        plan = algo.plan(context)
        if plan.intents:
            assert plan.intents[0].kind == "option"
            assert plan.metadata.get("macro") == "bull"
            assert plan.metadata.get("option_type") == "call"
    ...
    def test_bear_macro_picks_puts(self) -> None:
        ...
        plan = algo.plan(context)
        if plan.intents:
            assert plan.metadata.get("macro") == "bear"
            assert plan.metadata.get("option_type") == "put"

    def test_no_benchmark_data_returns_flat(self) -> None:
        algo = OptionsFlipAlgorithm({})
        context = AlgorithmContext(
            config=OptionsFlipConfig(benchmark_symbol="SPY"),
    ...
    def test_requirements_declares_intraday(self) -> None:
        ...
        reqs = algo.requirements(
            OptionsFlipConfig(intraday_bar_minutes=15, intraday_momentum_bars=4),
            {},
        )
    ...
    def test_sizing_returns_zero_thresholds(self) -> None:
        algo = OptionsFlipAlgorithm({})
        s = algo.sizing(OptionsFlipConfig())
...
class TestIntradayReplayStepping:
    def test_intraday_replay_steps_at_15min_intervals(self) -> None:
        ...
        algo = OptionsFlipAlgorithm({})
        # The replay steps its own intraday grid from ``intraday_minutes`` below; the class
        # cron only decides which *dates* run, and the default already runs every weekday.
        assert algo.cron == "*/15 9-15 * * 1-5"
        ...
        config_obj = Config(
            algorithm_configs={"options_flip": {}},
        )
    ...
    def test_daily_replay_still_works(self) -> None:
        ...
        trade_dates = sorted(set.intersection(*(set(df.index) for df in daily.values())))
        config_obj = Config(
            algorithm_configs={"bursty_dca": {"plan": {"buy": {"amount": 100, "items": []}, "sell": {"amount": 0, "items": []}}}},
        )
