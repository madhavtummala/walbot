...
whether a name may be held at all. A name failing here is never ranked. Each gate returns a
:class:`Check` (the measurement beside the threshold) rather than a bare boolean, and a verdict
is ``all(check.ok for check in ...)`` -- one list, so the reason a name was rejected is never
written twice and cannot drift from the test that produced it.
"""
...
def _volatility_checks(row: dict[str, Any], config: RallyRotationConfig) -> list[Check]:
    """A ceiling on how volatile a name may be. Entry only, like every gate here."""
    checks: list[Check] = []
...
def universe_data_ok(scored: dict[str, dict[str, Any]], config: RallyRotationConfig) -> dict[str, Any]:
    """Whether enough of the universe can be judged at all -- a data check, not a market view.
    Too little usable history is not a bearish reading, it's an unusable one; a cold cache would
    otherwise read as "nothing is above its average", a bear market that never happened.
    """
