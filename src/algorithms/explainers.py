...
EXPLAINERS: dict[str, dict[str, Any]] = {
    ...
    "options_flip": {
        ...
        "formula": [
            ...
            "  a threshold, not a rank: strength >= min_trend_strength proposes a call and",
            "  strength <= -min_bear_trend_strength proposes a put; between them, nothing is",
            "  a candidate, judged alone rather than against the others",
            "",
            "three gates, all in ATR of the symbol's own range -- never in percent:",
            "  1. thesis intact: a call needs price > SMA(regime_fast_ma_days) and price > VWAP",
            "     or recovering; a put is the mirror -- below the average, below VWAP or fading.",
            "     off the opening-range low, and the open is not an adverse gap beyond gap_atr",
            ...
            "  converges on the mark at the hold deadline, or once the regime gate has read",
        ...
        "parameters": {
            ...
            "min_bear_trend_strength": {"what": "The same threshold on the short side, as a positive number: a put needs strength at or below its negative.", "effect": "This and min_trend_strength bound the neutral band: a symbol scoring between them proposes no direction and arms nothing. Higher widens that band and shorts less. Deeper than the long side by default, since a bear reading fights positive drift and put skew prices the same move richer."},
            ...
            "gap_atr": {"what": "Largest opening gap AGAINST the thesis still an ordinary session, in ATR. One knob, mirrored: read as gap-down for a call and gap-up for a put.", "effect": "Lower refuses more opening gaps on either side. The favourable end is never guarded, since a gap that helps only makes the entry harder to fill and the reach probability already prices that."},
            ...
            "exit_trend_band_atr": {"what": "How far the wrong way price must travel past the fast average before a held position's thesis counts as broken, in ATR.", "effect": "Higher holds through bounces that poke back over the average and leaves the deadline or the stop to end the trade; lower concedes on the first crossing. At 0 one ordinary counter-trend day can close the gate and converge the exit to the mark."},
        },
    },
}
